...
const FooterForm: React.FC<FooterFormProps> = (props) => {
  const { onChange, setting = {}, disabled = false } = props;
  ...
  const onImageUploadComplete =
    (index: number) => (current: any, response: TPicture) => {
      let _links = [...(setting?.settingData?.links || [])];
      if (_links[index]) _links[index] = { ..._links[index], icon: response };
      // else
  ...
  const handleLinkRemove = (index: number) => () => {
    onChange(
      "links",
      setting?.settingData?.links.filter((_, i) => index !== i) || []
    );
  };
  ...
  return (
    <Paper>
      <Box p={3}>
        ...
        <Box my={2}>
          ...
          <Box my={1} display="flex" alignItems="center" width="100%">
            <IconButton disabled={disabled} size="small" onClick={newMediaData}>
              <Icon>add</Icon>
            </IconButton>
          </Box>
        </Box>
      ...
      {props.fields ? (
        <Paper
          className={props.variableContainerClass || classes.bodyFields}
          elevation={1}
        >
