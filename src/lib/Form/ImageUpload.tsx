...
function SingleImageUpload<T extends any>(props: IProps<T>) {
  ...
  const {
    mini = false,
    avatar = false,
    dimension = { width: "100%", height: "unset" },
    placeholderText = "",
    disabled = false,
  } = props;
  ...
  const uploadFiles = async (files: any[]) => {
    ...
    Promise.all(filePromises).then(async (newFiles) => {
  ...
  return props.loading ? (
    ...
    <Paper
      square={avatar ? false : true}
      style={{ borderRadius: avatar ? "50%" : undefined }}
    >
      {props.imageUrl ? (
        <img
          src={props.imageUrl}
      ...
      <div
        ...
        style={{ top: !!props.imageUrl ? undefined : "50%" }}
      >
        {!props.imageUrl ? (
          <>
        ...
        >
          <i style={mini ? { fontSize: 14 } : {}} className="material-icons">
            camera_alt
          </i>
          <FileInput accept="image/*" multiple={false} onDone={uploadFiles} />
        </Fab>
      </div>
    </Paper>
  );
